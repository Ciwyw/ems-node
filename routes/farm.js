...
const db = require('../db');
const { farm } = require('../db/sql');

router.prefix('/farm');

router.post('/add', async function (ctx) {
    const { farm_name, image, address, stall_num, temp_thres, humi_thres, illum_thres, amm_thres, h2s_thres, co2_thres } = ctx.request.body;
    const {err, results} = await db.query(farm.add, [farm_name, image, address, stall_num, temp_thres, humi_thres, illum_thres, amm_thres, h2s_thres, co2_thres]);
    if(err) {
